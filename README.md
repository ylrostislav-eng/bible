...
- ✅ Изучение: читалка (полный текст Синодального перевода, навигация по
  книгам и главам) + проверка по главе с таймером и очками за верные
  ответы + серия дней подряд (streak). Вопросы пока только для Послания
  Иакова — для остальных книг добавляются постепенно.
