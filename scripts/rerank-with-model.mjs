 ...
 *
 * Спрашивают не «насколько слова связаны», а «что напишет игрок», и это
 * не придирка к формулировке. На первом прогоне вопрос про связь дал
 * ровно то, что просили: к «Еве» модель подняла «марию» — параллель
 * между двумя Заветами, безупречная и совершенно бесполезная, — а
 * «ребро» вытеснила из первой десятки вовсе. К «Ною» подставила «исход».
 * Модель отвечала как богослов, а нам нужно, как играющий человек.
 */
function prompt(secret, words) {
  return `Идёт игра в «горячо-холодно». Загадано слово «${secret}».
Игрок пишет любые слова и узнаёт, насколько близко он к загаданному.

Оцени для каждого слова ниже, насколько вероятно, что человек напишет
именно его, думая о загаданном, от 0 до 10:
10 — первое, что приходит в голову (ковчег → потоп),
7 — назовут, перебрав очевидное (ковчег → смола),
4 — тема та же, но вспомнят не сразу (ковчег → море),
1 — почти никто не свяжет,
0 — не связано никак (ковчег → стул).

Оценивай ход мысли обычного человека, а не богословскую глубину. То,
что в этой истории видно и трогают руками, стоит выше тонкой параллели
между двумя книгами: к «Еве» ребро и яблоко важнее, чем Мария.
Многозначное слово понимай в том значении, которое подходит к
загаданному.
...
async function main() {
  ...
  // прервать его должно быть не страшно. Но когда меняется сама просьба к
  // модели, старые оценки отвечают на старый вопрос, и их надо выбросить:
  // `--fresh` для этого.
  const fresh = process.argv.includes('--fresh');
  if (fresh) {
    rmSync(PROGRESS_FILE, { force: true });
    console.log('Начинаю заново: прежние оценки отброшены.');
  }
  const done =
    !fresh && existsSync(PROGRESS_FILE)
