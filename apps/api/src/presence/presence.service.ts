...
export class PresenceService {
   ...
   *
   * Раньше уйти было нельзя — присутствие только истекало само через
   * минуту. Минута молчания сама по себе терпима, но она врёт в двух
   * заметных местах: сменивший аккаунт ещё числится в сети под прежним, а
   * уведомление о вызове не отправляется, потому что «он же в сети».
   * _Живой случай владельца: вышел с одного аккаунта, вошёл с другого — и
   * видел себя прежнего в сети._
   *
   * Истечение по времени при этом остаётся страховкой: если приложение
   * убили жёстко и сказать «ухожу» оно не успело, через минуту всё равно
   * станет тихо.
   */
