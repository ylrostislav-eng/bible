...
export interface FusionTuning {
   ...
   * такая же, как у трёх статистических мер, и считаться должна как они.
   */
  rerankSmoothing: number;
}
 ...
 *
 * Модель смягчается по-общему, вместе со статистикой. Сначала она делила
 * смягчение с выписанными связями, и это была ошибка на ровном месте: с
 * таким доверием её первая десятка становилась первой десяткой игры
 * целиком. Живой прогон показал, чем это кончается, — семимиллиардная
 * модель тянула наверх «ноем» к Адаму и «абрама» к Каину, слова, которых
 * ни одна другая мера рядом не видит. Как пятая мера среди четырёх она
 * своё влияние сохраняет, но переспорить остальные четыре в одиночку уже
 * не может.
 */
