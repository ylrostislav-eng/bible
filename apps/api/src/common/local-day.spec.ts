import { dayIndex } from './local-day';
...
describe('dayIndex', () => {
  ...
  it('за длину банка не повторяет ни одного слова', () => {
    ...
    for (let i = 0; i < BANK; i += 1) seen.add(dayIndex(day(i), BANK));
    expect(seen.size).toBe(BANK);
  });

  it('повторяет цикл ровно через длину банка', () => {
    expect(dayIndex(day(BANK), BANK)).toBe(dayIndex(day(0), BANK));
  });

  it('в один и тот же день даёт один и тот же ответ', () => {
    expect(dayIndex(day(7), BANK)).toBe(dayIndex(day(7), BANK));
  });

  it('соседние дни всегда разные', () => {
    for (let i = 0; i < 50; i += 1) {
      expect(dayIndex(day(i), BANK)).not.toBe(dayIndex(day(i + 1), BANK));
    }
  });

  it('не выходит за границы даже на банке из одного слова', () => {
    expect(dayIndex(day(5), 1)).toBe(0);
  });

  it('не даёт отрицательный индекс на датах до 1970 года', () => {
    const index = dayIndex(new Date(Date.UTC(1969, 0, 1)), BANK);
