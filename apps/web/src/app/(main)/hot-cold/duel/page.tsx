        ...
        <>
          ...
          {duelOver ? (
            ...
            <>
              ...
              <WordLookup state={state} onLookup={duel.lookup} />
 ...
 *
 * Не подсказка про загаданное слово, а помощь вспомнить сами слова.
 * Библейская лексика — главная стена режима: человек понимает, куда
 * думать, но не может вспомнить ни одного подходящего слова и стоит.
 *
 * Поиски личные и независимые: они никого ни к чему не приближают и
 * ничего не отнимают у соперника, поэтому согласовывать их не с кем — в
 * отличие от общих подсказок.
 */
function WordLookup({
  state,
  onLookup,
  ...
  // Свёрнут по умолчанию: экран дуэли и без того плотный, а словарь нужен
  // не каждый ход, а когда встал.
  if (!open) {
    return (
      <button
        type="button"
        ...
        className="self-center text-xs text-text-muted underline-offset-4 hover:text-text-secondary hover:underline"
      >
        Словарь · осталось {state.lookupsLeft}
      </button>
    );
  }
  ...
  return (
    <section className="flex flex-col gap-2 rounded-2xl border border-border bg-surface p-3">
      ...
      <p className="text-xs text-text-muted">
        Напишите любое слово — покажем пять близких по смыслу. Это не про загаданное слово, а чтобы
        вспомнить, чем вообще ходить.
      </p>
      {state.lookupsLeft > 0 && (
        <div className="flex gap-2">
          <input
            ...
            aria-label="Слово для словаря"
            className="h-10 flex-1 rounded-xl border border-border bg-surface px-3 text-sm outline-none transition focus:border-primary"
          />
          <button
            type="button"
            onClick={send}
            disabled={query.trim().length === 0}
            className="shrink-0 rounded-xl bg-surface-hover px-4 text-sm font-semibold transition hover:bg-border disabled:opacity-40"
          >
            Найти
          </button>
        </div>
      )}
      {state.lookups.map((entry, index) => (
        <div key={index} className="rounded-xl bg-surface-hover px-3 py-2">
          <p className="text-xs text-text-muted">{entry.query}</p>
          <p className="mt-0.5 text-sm">{entry.words.join(', ')}</p>
        </div>
      ))}
    </section>
  );
