...
export default function DicePage() {
  ...
  // делает `router.push('/play/dice')` — но если игрок и так уже стоит на
  // этой странице (листает меню «Кости», а вызов пришёл в это время),
  // переход на тот же самый путь ничего не меняет: страница не
  // перемонтируется, а её собственный стейт (`match`) как был `null`, так
  // и остаётся. Экран тихо стоит на меню, хотя за столом уже сидят —
  // ровно то же семейство бага, что и раньше, только с другим триггером
  // (не сервер не узнаёт активный стол, а клиент не замечает, что должен
  // его перечитать). _Нашлось живой проверкой личного вызова: соперник
  // принял приглашение с открытого меню «Кости», и стол у него на
  // сервере появился, а на экране — нет, пока не перезайти._
  useEffect(() => {
    if (activeGame?.type !== 'dice') return;
    if (match?.matchId === activeGame.sessionId) return;
    void apiClient
      .get<{ match: DiceMatchView | null }>('/dice/active')
      .then((active) => {
        if (active.match) apply(active.match);
    ...
    <div className="relative mx-auto h-[var(--app-height)] w-full max-w-md overflow-hidden">
      ...
      {!finished && !abandoned && (
        /* HUD привязан к самой сцене, а не к меняющей высоту колонке
           действий: свободная полоса столешницы между локтями и доской. */
        <div
          className="pointer-events-none absolute inset-x-0 top-[52.5%] z-10 px-3"
          data-testid="dice-score-hud"
        >
          <div className="flex h-[3.25rem] items-start gap-2">
            <ScoreChip player={me} label="Вы" active={visualMyTurn} target={match.targetScore} />
            <div className="shrink-0 rounded-full bg-black/70 px-2.5 py-1 text-center shadow-lg backdrop-blur-sm">
