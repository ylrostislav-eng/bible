...
/**
 * Сколько человек прямо сейчас ищут соперника.
 *
 * Показывается рядом с кнопкой «Найти соперника»: без этого числа нажатие
 * — ставка вслепую, и человек, простоявший в пустой очереди минуту,
 * решает, что режим сломан, а не что он пришёл первым.
 *
 * Считаются только те, кто **на связи**. Ожидающая партия остаётся в базе
 * и после того, как её владелец закрыл приложение, — до уборки. Считать
 * такие значило бы обещать соперника, которого нет.
 */
export interface WaitingOpponentsView {
  /** Всего ищущих в этом режиме. */
  total: number;
  /**
   * Разбивка по числу вопросов — только для дуэли по вопросам.
   *
   * Подбор сажает друг к другу только тех, кто просил одинаковое число
   * вопросов, поэтому «ищут трое» при выбранных десяти вопросах — неправда,
   * если все трое ждут двадцати. Ключ — число вопросов строкой.
   */
  byQuestionCount?: Record<string, number>;
}
