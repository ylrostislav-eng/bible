...
/**
 * Сколько попыток возвращает «вторая попытка» из лавки.
 *
 * Две, а не одна: одна попытка после пяти неудачных — это почти наверняка
 * шестая неудача, и покупка ощущается обманом. Две дают настоящий шанс
 * дойти, особенно если человек к тому моменту взял подсказки.
 */
export const DAILY_WORD_RETRY_ATTEMPTS = 2;

export const DAILY_WORD_NO_RETRY_MESSAGE = 'Вторых попыток не осталось — их можно купить в лавке';
export const DAILY_WORD_RETRY_NOT_NEEDED_MESSAGE = 'Попытки на сегодня ещё есть';
export const DAILY_WORD_ALREADY_SOLVED_MESSAGE = 'Слово уже угадано';
...
export interface DailyWordState {
  ...
  /**
   * Сколько «вторых попыток» лежит у игрока в запасе.
   *
   * Нужно самому экрану: предлагать докупить попытку имеет смысл только
   * тому, у кого она есть, а гонять за этим числом отдельный запрос в
   * лавку ради одной кнопки — лишний круг на горячем экране.
   */
