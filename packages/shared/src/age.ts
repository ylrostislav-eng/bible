...
/**
 * Отказ, когда взрослый пытается позвать ребёнка, который его не добавлял.
 *
 * Формулировка намеренно не говорит «это детский аккаунт»: иначе отказ сам
 * становится способом выяснить, кто в приложении ребёнок, — а это ровно то
 * знание, ради сокрытия которого детские аккаунты и убраны из списков.
 */
export const CANNOT_REACH_MESSAGE = 'Этот игрок недоступен для приглашений';
...
export const GUARDIAN_CONSENT_POINTS = [
  'Ребёнок будет играть только с теми, кого сам добавил в друзья',
  'Открытые комнаты незнакомых игроков скрыты',
  'Никнейм ребёнка не находится по частичному поиску — только по точному совпадению',
  'На любого игрока можно пожаловаться — жалобы разбирает модерация',
];
